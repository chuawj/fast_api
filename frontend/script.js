...
function setupPostForm() {
    ...
    if (form) {
        form.onsubmit = async function(e) {
            ...
            // localStorage에서 user_id 읽어서 int 변환
...
// 기존 회원가입, 로그인, 아이디찾기, 비밀번호변경 코드(변경 없음)
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 이하 기존 코드 (회원가입, 로그인, 아이디찾기, 비번변경 등) 그대로 유지
    // ... (생략, 위에 제공하신 코드와 동일) ...

    // (아래는 이전 코드 그대로 사용, 추가/수정 없음)
